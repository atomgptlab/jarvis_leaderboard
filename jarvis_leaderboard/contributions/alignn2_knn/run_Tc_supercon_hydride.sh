#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-Tc_supercon_hydride-dft_3d-test-mae.csv.zip
#   ALIGNN 2.0 (knn graph, cutoff 8.0) on dft_3d property 'Tc_supercon_hydride'.
set -e
...
WORK="$HERE/_work_Tc_supercon_hydride"; mkdir -p "$WORK"
# 1) build dft_3d + benchmark train/val/test split for this property
"$PY" "$HERE/build_property_data.py" Tc_supercon_hydride "$WORK"
cp "$HERE/config_Tc_supercon_hydride.json" "$WORK/config.json"
...
name="AI-SinglePropertyPrediction-Tc_supercon_hydride-dft_3d-test-mae.csv"
p[["id","prediction"]].to_csv("$HERE/"+name, index=False)
with zipfile.ZipFile("$HERE/AI-SinglePropertyPrediction-Tc_supercon_hydride-dft_3d-test-mae.csv.zip","w",zipfile.ZIP_DEFLATED) as z: z.write("$HERE/"+name, name)
os.remove("$HERE/"+name)
print("wrote", "AI-SinglePropertyPrediction-Tc_supercon_hydride-dft_3d-test-mae.csv.zip")
PYEOF
