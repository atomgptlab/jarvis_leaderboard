...
Adds a new AI/SinglePropertyClass dataset: eight binary phenotypes
predicted from Pfam protein-domain content, for 13,669 bacterial strains
(BacDive-AI release).

Two dataset variants are registered deliberately:
    bacdive_<trait>        random train/val/test (the usual protocol)
    bacdive_clade_<trait>  whole gene-content clades held out

The pair exists because the split protocol, not the model, dominates it:
...
def fit_multitask(tr):
    ...
    Xt = torch.tensor((Xv[tr] - mu) / sd)
    Yt = torch.tensor(Yv[tr])
    Mt = torch.tensor(M[tr].astype(np.float32))
    pw = torch.tensor(
        [
            ((M[tr][:, j] & (Yv[tr][:, j] == 0)).sum())
            / max((M[tr][:, j] & (Yv[tr][:, j] == 1)).sum(), 1)
            for j in range(len(TRAITS))
        ],
        dtype=torch.float32,
    )
    net = Net(Xv.shape[1], len(TRAITS))
...
for split_file, ds in [
    ...
    for j, t in enumerate(TRAITS):
        slug = SLUG[t]
        idx = {
            k: [i for i in v if M[i, j]]
            for k, v in [("train", tr), ("val", val), ("test", te)]
        }
        if len(idx["test"]) < 20:
            continue
        bench = {
            k: {str(X.index[i]): int(Yv[i, j]) for i in v}
            for k, v in idx.items()
        }
        write_zip(
            BENCH / f"{ds}_{slug}.json.zip",
            f"{ds}_{slug}.json",
            json.dumps(bench),
        )
        ...
        for i, gi in enumerate(idx["test"]):
            rows.append(
                f"{X.index[gi]}," f"{float(prob[list(te).index(gi), j] > 0.5)}"
            )
        name = f"AI-SinglePropertyClass-{slug}-{ds}-test-acc.csv"
        write_zip(
            LB / "contributions/g2p_multitask" / (name + ".zip"),
            name,
            "\n".join(rows) + "\n",
        )

        lr = make_pipeline(
            StandardScaler(with_mean=False), LogisticRegression(max_iter=2000)
        )
        lr.fit(Xv[idx["train"]], Yv[idx["train"], j].astype(int))
        pl = lr.predict(Xv[idx["test"]])
        rows = ["id,prediction"] + [
            f"{X.index[gi]},{float(v)}" for gi, v in zip(idx["test"], pl)
        ]
        write_zip(
            LB / "contributions/g2p_logistic" / (name + ".zip"),
            name,
            "\n".join(rows) + "\n",
        )
        print(
            f"  {slug:14s} train {len(idx['train']):5d} "
            f"test {len(idx['test']):5d}",
            flush=True,
        )
print("done")
