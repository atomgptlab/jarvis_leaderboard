#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-bandgap-twod_matpd-test-mae.csv.zip
#   ALIGNN 2.0 (radius graph, cutoff 5.0) on jarvis 'twod_matpd' property 'bandgap'.
set -e
...
WORK="$HERE/_work_twod_matpd_bandgap"; mkdir -p "$WORK"
# 1) build id_prop from the leaderboard benchmark split (structures from jarvis 'twod_matpd')
"$PY" "$HERE/build_lb_data.py" twod_matpd bandgap "$WORK"
cp "$HERE/config_twod_matpd_bandgap.json" "$WORK/config.json"
# 2) train
...
name="AI-SinglePropertyPrediction-bandgap-twod_matpd-test-mae.csv"
