#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-optb88vdw_total_energy-dft_3d-test-mae.csv.zip
#   ALIGNN 2.0 (radius graph, cutoff 5.0) on dft_3d property 'optb88vdw_total_energy'.
set -e
...
WORK="$HERE/_work_optb88vdw_total_energy"; mkdir -p "$WORK"
# 1) build dft_3d + benchmark train/val/test split for this property
"$PY" "$HERE/build_property_data.py" optb88vdw_total_energy "$WORK"
cp "$HERE/config_optb88vdw_total_energy.json" "$WORK/config.json"
...
name="AI-SinglePropertyPrediction-optb88vdw_total_energy-dft_3d-test-mae.csv"
p[["id","prediction"]].to_csv("$HERE/"+name, index=False)
with zipfile.ZipFile("$HERE/AI-SinglePropertyPrediction-optb88vdw_total_energy-dft_3d-test-mae.csv.zip","w",zipfile.ZIP_DEFLATED) as z: z.write("$HERE/"+name, name)
os.remove("$HERE/"+name)
print("wrote", "AI-SinglePropertyPrediction-optb88vdw_total_energy-dft_3d-test-mae.csv.zip")
PYEOF
