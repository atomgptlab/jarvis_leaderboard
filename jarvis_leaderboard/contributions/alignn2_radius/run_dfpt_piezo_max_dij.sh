#!/bin/bash
# Reproduce AI-SinglePropertyPrediction-dfpt_piezo_max_dij-dft_3d-test-mae.csv.zip
#   ALIGNN 2.0 (radius graph, cutoff 5.0) on dft_3d property 'dfpt_piezo_max_dij'.
set -e
...
WORK="$HERE/_work_dfpt_piezo_max_dij"; mkdir -p "$WORK"
# 1) build dft_3d + benchmark train/val/test split for this property
"$PY" "$HERE/build_property_data.py" dfpt_piezo_max_dij "$WORK"
cp "$HERE/config_dfpt_piezo_max_dij.json" "$WORK/config.json"
...
name="AI-SinglePropertyPrediction-dfpt_piezo_max_dij-dft_3d-test-mae.csv"
p[["id","prediction"]].to_csv("$HERE/"+name, index=False)
with zipfile.ZipFile("$HERE/AI-SinglePropertyPrediction-dfpt_piezo_max_dij-dft_3d-test-mae.csv.zip","w",zipfile.ZIP_DEFLATED) as z: z.write("$HERE/"+name, name)
os.remove("$HERE/"+name)
print("wrote", "AI-SinglePropertyPrediction-dfpt_piezo_max_dij-dft_3d-test-mae.csv.zip")
PYEOF
